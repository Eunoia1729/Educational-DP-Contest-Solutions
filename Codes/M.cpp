...
ll sum_query(ll ind, ll l, ll r) {
    if( r < l) {
        return 0;
    }
    return ( query(ind, r) + mod - query(ind, l - 1)) % mod;
}
int main()
{
    #ifndef ONLINE_JUDGE
    ...
    rep(i, 1, n + 1) {
        rep2(j, k, 0) {
            diff = sum_query(i - 1, max(j - v[i - 1], 0LL), j);
            update(i, j, diff);     
        }
    }
    cout << sum_query(n, k, k) << endl;
    return 0;    
}